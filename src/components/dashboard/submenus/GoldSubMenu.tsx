...
import { ArrowLeft, Coins, Edit2, Trash2, Eye } from 'lucide-react';
...
export const GoldSubMenu: React.FC<GoldSubMenuProps> = ({ 
  savings, 
  rates, 
  onBack, 
  displayCurrency,
  ...
  const goldSavings = useMemo(() => 
    savings.filter(s => s.type === SavingType.GOLD),
  [savings]);

  const filteredGold = useMemo(() => {
    if (!filterName) return goldSavings;
    return goldSavings.filter(s => s.name === filterName);
  }, [goldSavings, filterName]);

  const goldData = useMemo(() => {
    const byName: Record<string, number> = {};
    goldSavings.forEach(s => {
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">Active în Aur</h2>
          <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Detaliere portofoliu metale prețioase</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col items-center">
        {goldData.length > 0 ? (
          <PieChartWithLegend 
            data={goldData}
            colors={COLORS}
            title={filterName ? `Filtru: ${filterName}` : "Distribuție Aur"}
            centerLabel={filterName ? filterName : `${goldSavings.length} Poziții`}
            centerSubLabel="Aur"
      ...
      <div ref={listRef} className="space-y-4 pt-4">
        {filteredGold.map(s => (
          <div key={s.id} className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 flex justify-between items-center group/item hover:border-amber-500 transition-all relative overflow-hidden">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-amber-50 dark:bg-amber-500/10 text-amber-500 rounded-xl group-hover/item:bg-amber-500 group-hover/item:text-white transition-all">
                <Coins className="w-5 h-5" />
              </div>
              <div>
                <p className="font-black text-slate-900 dark:text-white">{s.name}</p>
                <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                  Aur • {(s as any).weightInGrams || 0}g
                </p>
              </div>
            </div>
