...
export const SavingForm: React.FC<SavingFormProps> = ({ onAdd, onCancel }) => {
  const [type, setType] = useState<SavingType>(SavingType.DEPOSIT);
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          {SAVING_OPTIONS.map((opt) => (
            <button
              ...
              className={cn(
                "flex flex-col items-center justify-center gap-2 min-w-[80px] p-3 rounded-2xl border transition-all text-[10px] font-bold uppercase tracking-tight",
                type === opt.type 
